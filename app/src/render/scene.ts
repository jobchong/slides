  ...
  if (isLine) {
    ...
    lineSvg = `
      <svg viewBox="0 0 100 100" preserveAspectRatio="none" style="width:100%; height:100%; display:block;">
        <line x1="${lineStart.x}" y1="${lineStart.y}" x2="${lineEnd.x}" y2="${lineEnd.y}" stroke="${stroke}" stroke-width="${strokeWidth}" stroke-linecap="${lineCap}"${dashAttr} vector-effect="non-scaling-stroke"></line>
      </svg>
    `.trim();
  }
